...
#set arduino connection
ser = serial.Serial('/dev/ttyACM0', 9600)


# or local ip of MATRIX creator
creator_ip = os.environ.get('CREATOR_IP', '192.168.0.12')
...
up_message = '{"receiver":"HEXAPOD_ID","sender":"HOLOLENS_ID","body_message":{"func":"hexapodUpDown","parameters":{"V_right_joystick":128,"H_right_joystick":128,"V_left_joystick":128,"H_left_joystick":128,"buttons":16}}}'
down_message ='{"receiver":"HEXAPOD_ID","sender":"HOLOLENS_ID","body_message":{"func":"hexapodUpDown","parameters":{"V_right_joystick":128,"H_right_joystick":128,"V_left_joystick":128,"H_left_joystick":128,"buttons":16}}}'
...
effet_dispo=['clignote','tourne','spirale','pulsation','volet']
move_robot_qr=['avance','recule','droite','gauche','debout','couche','tourne_droite','tourne_gauche','pince_mqtt','bras_haut','bras_bas','stop']
move_robot_mqtt=[forward_message,backward_message,right_message,left_message,up_message,down_message,right_rotation_message,left_rotation_message,plier_message,arm_up,arm_down,stop_message] #Attention : penser a faire correspondre les positions des instruction QR et MQTT
#move_robot_mqtt=[forward_message,backward_message,right_message,left_message,up_message,down_message,right_rotation_message,left_rotation_message,stop_message]
move_head=['penche_haut','penche_bas','rotation_droite','rotation_gauche','monte','descend','pince']
...
if __name__ == '__main__':
    ...
    while True :
        camera.capture('zbar-test.jpg') #mind the focus
	...
	#tram for test without camera 
        ...
        if(len(tram)>0):
            tram=tram.decode('utf-8')
            tram=tram.split('/') #for each steps
...
'''
	...
	tram='rouge(12),clignote(41),penche_haut(2),avance(5)/delay(4)/recule(8),penche_bas(4)' #for test tram function without QR code
	tram=tram.split('/')
	instruction=[]
	...
	for step in tram :
		...
		#create_instruction(step,'rouge',etape)
            	#create_instruction(step,'clignote',etape)
            	#create_instruction(step,'penche_haut',etape)
